...
function handleFileSelection(event) {
    ...
    if (files.length === 0) return;
        ...
        // Skip files without dates for GPX matching
        if (!dateTaken) {
            logActivity(`⚠️ Skipping ${file.name} - no date found in filename or EXIF data`, 'warning');
            return null;
        }
        ...
        const entry = {
            ...
            dateTakenLocal: new Date(dateTaken.getTime() + CEST_OFFSET_MS)  // Local time for display
        };
        return entry;
    })).then(fileData => {
...
function handleMediaDrop(e) {
    ...
    Promise.all(files.map(async file => {
        ...
        let dateTaken = FilenameDateParser.parse(file.name);
        if (!dateTaken) {
            dateTaken = new Date(file.datetaken || file.lastModified);
        }
