...
function showMediaPins() {
    ...
    mediaFiles.forEach((file, index) => {
        if (file.coordinates) {
            ...
            markerEl.innerHTML = `${file.type === 'image' ? STYLE_MEDIA_MARKER.emojiImage : STYLE_MEDIA_MARKER.emojiVideo}<div style="position: absolute; top: -10px; right: -10px; background: #333; color: white; font-size: 12px; font-weight: bold; padding: 0; border-radius: 10px; min-width: 20px; text-align: center; line-height: 20px; z-index: 9999;">${index + 1}</div>`;
                    ...
                    const onIdle = () => {
                        ...
                        try {
                            ...
                            mediaFeatures.forEach((f, index) => {
                                ...
                                // Draw marker circle (consistent size)
                                ...
                                // Draw ID badge (top-right of marker)
                                const badgeSize = 16;
                                const badgeX = p.x + markerRadius + 2;
                                const badgeY = p.y - markerRadius - 2;
                                
                                // Draw badge background
                                ctx2d.fillStyle = '#333';
                                ctx2d.fillRect(badgeX, badgeY, badgeSize, badgeSize);
                                ...
                                ctx2d.fillText(index + 1, badgeX + badgeSize/2, badgeY + badgeSize/2);
                            });
