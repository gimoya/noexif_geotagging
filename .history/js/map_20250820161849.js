...
function updatePositionMarker(fileIndex, coords) {
    ...
    markerEl.innerHTML = `${STYLE_POSITION_MARKER.emoji}<div style="position: absolute; top: -8px; right: -8px; background: #333; color: white; font-size: 10px; font-weight: bold; padding: 1px 4px; border-radius: 8px; min-width: 16px; text-align: center;">${fileIndex + 1}</div>`;
...
function showMediaPins() {
    ...
    mediaFiles.forEach((file, index) => {
        if (file.coordinates) {
            ...
            markerEl.innerHTML = `${file.type === 'image' ? STYLE_MEDIA_MARKER.emojiImage : STYLE_MEDIA_MARKER.emojiVideo}<div style="position: absolute; top: -8px; right: -8px; background: #333; color: white; font-size: 10px; font-weight: bold; padding: 1px 4px; border-radius: 8px; min-width: 16px; text-align: center;">${index + 1}</div>`;
