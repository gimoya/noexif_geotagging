                    ...
                    const onIdle = () => {
                        ...
                        try {
                            ...
                            mediaFeatures.forEach((f, index) => {
                                ...
                                // Draw ID badge (above marker, centered)
                                const badgeSize = 20;
                                const badgeX = p.x - badgeSize/2; // Center horizontally on marker
                                const badgeY = p.y - markerRadius - badgeSize - 2; // Above marker
                                
                                // Draw badge text (no background, transparent)
                                ctx2d.font = 'bold 14px sans-serif';
                                ctx2d.fillStyle = '#333'; // Dark text for visibility
                                ctx2d.textAlign = 'center';
                                ctx2d.textBaseline = 'middle';
                                ctx2d.fillText(index + 1, p.x, p.y - markerRadius - 10);
                            });
